...
description: "Try[T] is iter.Seq2[T, error]: a lazy sequence whose elements each either succeeded or carry an error. The design's central choice is that the pipeline does not decide what an error means — the consumer does, because both policies are legitimate in the same codebase: a malformed line in a log scan should probably be skipped, a failed row scan should probably abort."
sidebar:
  order: 3
---
